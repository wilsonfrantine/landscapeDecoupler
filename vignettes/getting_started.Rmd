...
plot_decoupled(multiscales$p01,
               legend = T,
               colors = colors$hex, 
               breaks = breaks)
```
...
b<-c(250, 500, 1000, 1500)
specific.scales <- decouple(r, p, b, specific = c(2))
...
plot_decoupled(specific.scales[[1]], 
               ...
               colors = colors$hex, 
               breaks = breaks)
#you defined breaks values up above with c(as.numeric(levels(as.factor(values(r)))))
```
...
We can provide the output from the `LandscapeDecoupled` as input into functions as `calculate_lsm`, since the output is a raster object. Users might do this as follow:
...
As you can see, `landscapeMetrics` can read the layers in raster contained into `specific.scales[[1]]` index or `specific.scales$p01`. Therefore, the first layer represents the s1_500 and the layer two is the mergeing of X250, X1000, and X1500.
