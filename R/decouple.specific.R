...
decouple.specific <- function(ls=NULL, specific=NULL){
  ...
  else if(length(specific) >= raster::nlayers(ls[[1]])-1) stop("you are trying to keep so many scales as layers (or `n` scales - 1) in the raster object. It will result in the same object you already have. \n Execution helted!")
